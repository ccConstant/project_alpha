...
class CompFamilyController extends Controller
{
     ...
     * @return Response : id of the new comp family
     */
    public function add_compFamily(Request $request)
    {
        ...
        $compFamily = compFamily::create([
            ...
            'compFam_mainRef' => $request->artFam_mainRef,
        ]);
    ...
    public function send_compFamilies()
    {
        ...
        foreach ($compFamilies as $compFamily) {
            ...
            $obj = [
                ...
                'compFam_mainRef' => $compFamily->compFam_mainRef,
            ];
    ...
    public function send_compFamily($id)
    {
        ...
        $mainDesign = null;
        if ($compFamily->compFam_mainRef !== NULL) {
            $comp = CompFamily::all()->where('compFam_ref', '=', $compFamily->compFam_mainRef);
            $cons = ConsFamily::all()->where('consFam_ref', '=', $compFamily->compFam_mainRef);
            $raw = RawFamily::all()->where('rawFam_ref', '=', $compFamily->compFam_mainRef);
            if (count($comp) !== 0) {
        ...
        $obj = [
            ...
            'compFam_updated_at' => $compFamily->updated_at,
            'compFam_mainRef' => $compFamily->compFam_mainRef,
            'compFam_mainRefDesign' => $mainDesign,
        ];
