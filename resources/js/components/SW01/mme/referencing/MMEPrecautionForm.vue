    ...
    <div :class="divClass">
        ...
        <div v-else>
            <form class="container" @keydown="clearError">
                ...
                <InputSelectForm v-model="prctn_type" :Errors="errors.prctn_type"
                                 :id_actual="precautionType" :info_text="infos_prctn[0].info_value" :isDisabled="!!isInConsultedMod"
                                 :options="enum_prctn_type" :selctedOption="this.prctn_type" :selectedDivName="this.divClass"
                                 label="Precaution type :" name="prctn_type" selectClassName="form-select w-50"
                                 @clearSelectError='clearSelectError'/>
                <InputTextAreaForm v-model="prctn_description" :Errors="errors.prctn_description"
                                   :info_text="infos_prctn[1].info_value" :isDisabled="!!isInConsultedMod" inputClassName="form-control w-50"
                                   label="Description :" name="prctn_description"/>
                    ...
                    <div v-if="this.prctn_id==null ">
                        <div v-if="modifMod==true">
                            <SaveButtonForm :AddinUpdate="true" :consultMod="this.isInConsultedMod"
                                            :savedAs="prctn_validate" @add="addMMEPrctn"
                                            @update="updateMMEPrctn"/>
                        </div>
                        <div v-else>
                            <SaveButtonForm :consultMod="this.isInConsultedMod" :savedAs="prctn_validate"
                                            @add="addMMEPrctn" @update="updateMMEPrctn"/>
                        </div>
                    </div>
                    <div v-else-if="this.prctn_id!==null">
                        <SaveButtonForm :consultMod="this.isInConsultedMod" :modifMod="this.modifMod" :savedAs="prctn_validate"
                                        @add="addMMEPrctn" @update="updateMMEPrctn"/>
                    </div>
                    <!-- If the user is not in the consultation mode, the delete button appear -->
                    <DeleteComponentButton :consultMod="this.isInConsultedMod" :validationMode="prctn_validate"
