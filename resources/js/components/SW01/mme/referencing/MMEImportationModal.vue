...
<template>
    <div>
        ...
        <b-modal id="modal-mme_add_import" hide-footer title="Importation">
            <p class="my-4">Import from set : {{ set }}</p>
            <div>
                <div v-if="imported_mme_ref.length>0">
                    <input v-model="searchTerm" type="text">
                    <div v-for="option in filterByTerm" :key="option.id">
                        <input :id="option.mme_internalReference" v-model="radio_value" :value="option" name="radio-input"
                               type="radio"/>
            ...
            <b-button block class="mt-3" @click="$bvModal.hide('modal-mme_add_import')">Close</b-button>
