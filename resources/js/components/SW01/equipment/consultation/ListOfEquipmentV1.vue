...
<template>
    <div class="listOfEquipment">
                ...
                <div v-for="(list,index) in pageOfItems " :key="index" class="one_element_list">
                    <li :class="'element'+index%2" class="list-group-item">
                        ...
                        <div class="eq_list_option">
                            ...
                            <a href="#"
                               @click="warningUpdate(list.id,list.alreadyValidatedTechnical,list.alreadyValidatedQuality)">Update</a>
                            <a v-if="list.alreadyValidatedTechnical===false" href="#"
                               @click="technicalValidation(list.id)">Technical validation</a>
                            <a v-if="list.alreadyValidatedQuality===false" href="#" @click="qualityValidation(list.id)">Quality
                                validation</a>
                            <a v-if="list.alreadyValidatedTechnical===true && list.alreadyValidatedQuality===true">Statut
                                : Approved</a>
                            <a href="#" @click="reformEquipment(list.id)">Reform</a>
                            <router-link :to="{name:'url_lifesheet_pdf',params:{id: list.id} }">Generate PDF
                            </router-link>
                        </div>
                    </li>
                </div>
            </ul>
            <jw-pagination :items="filterByTerm" :pageSize=10 class="eq_list_pagination"
        ...
        <div class="router-link">
            <router-link :to="{name:'url_eq_list_pdf',params:{} }">
                <button class="btn btn-primary">GO TO PDF</button>
            </router-link>
        </div>
