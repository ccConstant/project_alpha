...
<!----------Props of other component who can be called:--------
    See details in related Vue Component
        EquipmentPowForm : type, name, value, unit,consumptionUnit,consumptionValue, validate, consultMod, modifMod, divClass, id, eq_id
...
<template>
    <div class="equipmentPow">
        ...
        <EquipmentPowForm ref="ask_pow_data" v-for="(component, key) in components" :key="component.key"
            ...
            :consumptionUnit="component.consumptionUnit" :divClass="component.className" :id="component.id"
        ...
        <SaveButtonForm saveAll v-if="components.length>2" @add="saveAll" @update="saveAll" :consultMod="this.consultMod" :modifMod="this.modifMod"/>
...
export default {
    ...
    methods: {
        ...
        //Function for adding imported power form with his data
        ...
        //Suppresion of a power component from the vue
        getContent(key) {
    ...
    created(){
        ...
        if(this.import_id!==null){
            ...
            var consultUrl = (id) => `/power/send/${id}`;
