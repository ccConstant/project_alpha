<template>
    <div class="equipmentSpProc" >
        ...
        <EquipmentSpecProcForm ref="ask_spProc_data" v-for="(component) in components" :key="component.key"
            :is="component.comp" :divClass="component.className" :id="component.id" :exist="component.exist"
            :remarksOrPrecaution="component.remarksOrPrecaution" :name="component.name" :validate="component.validate" 
            :consultMod="isInConsultMod" :modifMod="isInModifMod" :eq_id="data_eq_id"/>
        <!--If the user is not in consultation mode -->
        <div v-if="!this.consultMod">
            <!--If special process array is not empty and if the user is not in modifacion mode -->
...
export default {
    ...
    methods: {
        ...
        //Function for adding imported special process form with his data
        ...
        importSpProc(){
            ...
            if(this.specProc.length>0){
                var className="importedSpProc"+this.specProc[0].id
    ...
    created(){
        ...
        if(this.import_id!==null){
            //Make a get request to ask to the controller the special process corresponding to the id of the equipment with which data will be imported
            var consultUrl = (id) => `/spProc/send/${id}`;
    ...
    mounted(){
        ...
        //If the user is in consultation or modification mode special process will be added to the vue automatically
